  ...
  <profiles>    
   ...
   能够在某些特定的环境中自动使用某些特定的值；这些环境通过activation元素指定。activation元素并不是激活profile的唯一方式。-->
      ...
      <activation>        
        ...
        <!-- property 如果Maven检测到某一个属性（其值可以在POM中通过${名称}引用），其拥有对应的名称和值，Profile就会被激活。如果值
...
# 参考资料
- 《Maven 实战》（第1版），许晓斌著
- Maven系列--pom.xml 配置详解, longeremmy的专栏
